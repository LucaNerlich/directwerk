...
3. Set `OAUTH_CLIENT_ID=publish-tenant-frontend` and `OAUTH_CLIENT_SECRET` to
   `DIRECTWERK_TENANT_CLIENT_SECRET` from Directwerk’s `.env`.
   Do **not** use `publish-platform-admin` here — that client is for `publish-admin` only.
...
Published content from **publish-studio** is readable here without login:

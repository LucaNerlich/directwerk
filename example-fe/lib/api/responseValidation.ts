...
export function parseSiteConfigEnvelope(
    ...
    return envelope(value, (data) => {
        ...
        const publicRssUrl =
            data.publicRssUrl === undefined || data.publicRssUrl === null
                ? null
                : isBoundedString(data.publicRssUrl, 4096) &&
...
function parseAccessPackage(value: unknown): AccessPackage | null {
    if (
        !isRecord(value) ||
        !isPositiveSafeInteger(value.id) ||
        !isBoundedString(value.slug) ||
...
export function parseAccessEnvelope(value: unknown): ApiEnvelope<Access> | null {
    return envelope(value, (data) => {
        ...
        const activeLevels: AccessLevel[] = []
        for (const level of data.activeLevels) {
            const parsed = parseAccessLevel(level)
            if (parsed === null) {
                return null
            }
            activeLevels.push(parsed)
        }

        const packagesRaw = Array.isArray(data.activePackages)
            ? data.activePackages
            : []
        if (packagesRaw.length > 100) {
            return null
        }
        const activePackages: AccessPackage[] = []
        for (const item of packagesRaw) {
            const parsed = parseAccessPackage(item)
            if (parsed === null) {
                return null
            }
            activePackages.push(parsed)
        }
...
function parsePublicCategory(value: unknown): PublicCategory | null {
    if (
        ...
        !(
            ...
            isPositiveSafeInteger(value.parentId)
        )
    ) {
        return null
    }
...
function parsePublicFormat(value: unknown): PublicFormat | null {
    ...
    const description =
        value.description === undefined
            ? null
            : isNullableString(value.description, 4096)
              ? value.description
              : null
    if (value.description !== undefined && description === null && value.description !== null) {
        return null
    }
...
export function parsePublicFormatListEnvelope(
    ...
    return envelope(value, (data) =>
        parseBoundedArray(data, 200, parsePublicFormat),
    )
}
...
export function parsePublicCategoryListEnvelope(
    ...
    return envelope(value, (data) =>
        parseBoundedArray(data, 200, parsePublicCategory),
    )
}
...
function parsePublicProduct(value: unknown): PublicProduct | null {
    if (
        ...
        !isBoundedString(value.billingInterval, 16)
    ) {
        return null
    }
...
function parsePublicEpisode(value: unknown): PublicEpisode | null {
    ...
    const formatsRaw = Array.isArray(value.formats) ? value.formats : []
    const categoriesRaw = Array.isArray(value.categories) ? value.categories : []
    if (formatsRaw.length > 100 || categoriesRaw.length > 100) {
        return null
    }
    const formats: PublicFormat[] = []
    for (const item of formatsRaw) {
        const parsed = parsePublicFormat(item)
        if (parsed === null) {
            return null
        }
        formats.push(parsed)
    ...
    for (const item of categoriesRaw) {
        const parsed = parsePublicCategory(item)
        if (parsed === null) {
            return null
        }

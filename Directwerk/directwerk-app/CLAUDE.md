...
- `de.pnnit.directwerk.config.*` — `JpaAuditingConfig`, `CacheConfig`, `ApplicationConfig`, `DevDataInitializer` (dev-only seed data), `PlatformAdminBootstrap`, and the `Prod*PropertiesValidator` classes that fail fast at startup in `prod` if required security/email/storage properties are missing.

...
- `de.pnnit.directwerk.modules.core.util` — small helpers shared across modules: `EmailNormalizer`, `PasswordPolicy`, `SlugNormalizer`, `TokenHashUtil`, `TenantHostname`, `TenantAssetKeys`, `EnvelopeCipher`, and `EnvelopeTokenProtector` (the shared signed-envelope token seam that feed/email adapters delegate to).

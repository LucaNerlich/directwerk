# Directwerk — Media upload & read (how-to)

Operator guide for Phase **2c** (presigned upload + confirm) and **2d** (private signed GET).
Design details: [`../../docs/asset-storage.md`](../../docs/asset-storage.md). HTTP harness:
[`../http/17-media-upload.http`](../http/17-media-upload.http).
...
checklist:
[`bunny-net-integration.md` — CDN Pull Zones](../../docs/bunny-net-integration.md#implementation-guide-cdn-pull-zones).
When both private CDN vars are set, paid downloads use Bunny Advanced tokens; otherwise S3
presign.
...
`https://{location}.your-objectstorage.com` endpoint. Full variable list: [`.env.example`](../.env.example).

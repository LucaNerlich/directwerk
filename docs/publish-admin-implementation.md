# Publish — `publish-admin` Implementation Guide
...
for building the platform operations console at `projects/publish/publish-admin/`.
...
| [`publish-studio-implementation.md`](publish-studio-implementation.md) | Tenant dashboard (contrast — different auth boundary) |
| **This document** | **How to scaffold and implement** `publish-admin` |
| [`http/`](../http/) | Executable API acceptance criteria |

**Status (2026-08):** Implemented at `projects/publish/publish-admin/`, promoted from the
...
| **OAuth2 client `publish-platform-admin`** | Separate from tenant frontend client |
| **Invite-only accounts** | No public registration for `PLATFORM_ADMIN` |
| **No per-tenant theming** | Consistent ops UI — not whitelabel |
| **Shared UI system** | Tailwind CSS v4 and `@publish/ui`, consistent with studio and web |

### What publish-admin is not

| Misconception | Reality |
|---------------|---------|
| Tenant creator dashboard | That is `publish-studio` (`TENANT_ADMIN` / `EDITOR`) |
...
flowchart LR
    ...
    subgraph admin [publish-admin Next.js]
...
### Auth boundary vs `publish-studio`

| Concern | `publish-admin` | `publish-studio` |
|---------|-----------------|------------------|
| Domain | `admin.{platform}.de` | `studio.{tenant}.de` |
| OAuth client | `publish-platform-admin` | `publish-tenant-frontend` |
...
```
projects/publish/publish-admin/
  package.json
  tsconfig.json
  next.config.ts
  app/
...
NEXT_PUBLIC_OAUTH_CLIENT_ID=publish-platform-admin
# Server-side only:
# OAUTH_CLIENT_SECRET=publish-platform-admin-secret
```
...
sequenceDiagram
    ...
    participant App as publish-admin
    participant API

    Admin->>App: Enter email + password
    App->>API: POST /oauth2/token (password grant)
    Note over App,API: client: publish-platform-admin<br/>NO Host header
...
1. Login as `platform-admin@publish.local`
...
| Host | Target |
|------|--------|
| `admin.{platform}.de` | `publish-admin` container |
...
- [x] Next.js 16 project under `projects/publish/publish-admin/`
...
- Tenant dashboard contrast: [`publish-studio-implementation.md`](publish-studio-implementation.md)

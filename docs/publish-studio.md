# Publish — `publish-studio` (Creator Dashboard)

Companion to [`README.md`](../README.md) (full platform design). This document defines **`publish-studio`**
...
| **This document** | What `publish-studio` is, who it serves, creator journeys |
| [`publish-studio-implementation.md`](publish-studio-implementation.md) | **Implementation guide** — screens, API mappings, scaffold, auth, checklist |
...
**Status (2026-08):** Usable MVP on `@publish/ui`. Studio desks (Write + Podcast), media library,
products/subscribers, and the `publish-web` subscriber loop (account, feeds, catalog) are shipped.
...
See [`publish-studio-implementation.md` § Phased delivery](publish-studio-implementation.md#phased-delivery).

---

## What `publish-studio` is

**`publish-studio`** is the tenant **publisher back-office** — a web app where creators and their
...
| A subscriber-facing site | Subscribers use **`publish-web`** (or a custom frontend) — not studio |
| Platform superadmin console | That is **`publish-admin`** (`PLATFORM_ADMIN` only) |
...
|----------|-------------|-------|
| **Non-technical creators** (default) | `publish-studio` + `publish-web` | “Create everything in one dashboard on your domain” |
...
|-----|----------------|----------|------|
| **`publish-studio`** | `https://studio.mein-podcast.de` or `https://mein-podcast.de/studio` | `EDITOR`, `TENANT_ADMIN` | **Create and publish** — back-office |
| **`publish-web`** | `https://mein-podcast.de` | `GUEST`, `SUBSCRIBER` | **Consume** — public site, pricing, login, subscriber portal |
| **`publish-admin`** | `https://admin.{platform}.de` | `PLATFORM_ADMIN` | **Operate platform** — tenants, modules (internal only) |
...
flowchart TB
    ...
    subgraph audience_side [Audience side]
        Web[publish-web public]
        Portal[publish-web subscriber portal]
...
**Bundled default:** New tenants get `publish-studio` + `publish-web` on their domain. Custom
frontends remain supported for agencies — they replace `publish-web`, not the entitlement engine.

**Planned location:** `projects/publish-studio/` (dedicated app) **or** `/studio/**` routes inside
`projects/publish-web/`. Prefer a dedicated app when a customer wants publisher tools without the
public marketing site on the same deployable. See
[`publish-studio-implementation.md` § What this is](publish-studio-implementation.md#what-this-is-and-is-not).
...
3. Publish API fires domain event → ESP sends to free/paid segments
...
| **`publish-web` as default tenant site** | They need a public website without hiring an agency |
...
auth, and architecture: [`publish-studio-implementation.md`](publish-studio-implementation.md).
...
- Public + subscriber site: [`README.md` § Reference Frontend (publish-web)](../README.md#reference-frontend-publish-web)

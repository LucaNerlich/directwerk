# Publish — `publish-studio` Implementation Guide

Companion to [`publish-studio.md`](publish-studio.md) (product overview) and
[`README.md`](../README.md) (full platform design). This document is the **single implementation
spec** for the creator dashboard at `projects/publish-studio/` — product context, screen-by-screen
...
| **This document** | **How to build** `publish-studio` — screens, API mappings, scaffold, checklist |
...
**Status (2026-08):** Usable MVP at `projects/publish/publish-studio/` on `@publish/ui`. Write and
Podcast desks, media library, products/subscribers/grants, and `publish-web` subscriber feeds/account
...
|------------|----------|-------|
| **`publish-studio`** (this doc) | `TENANT_ADMIN`, `EDITOR` | One tenant — content, assets, team, subscribers, products |
| `publish-admin` | `PLATFORM_ADMIN` | All tenants — create/suspend, module toggles, invite tenant admins |
| `publish-web` subscriber portal | `SUBSCRIBER` | Own account — feeds, access, checkout (not publisher ops) |
| `publish-web` public site | `GUEST` | Marketing, free episodes, pricing |
...
**Planned location:** `projects/publish-studio/` (dedicated publisher app) **or** `/studio/**`
routes inside `projects/publish-web/`. Prefer a dedicated app when a customer wants publisher tools
...
| **OAuth2 JWT** | Client `publish-tenant-frontend`; stateless `Authorization` header |
...
- Not a subscriber-facing site → `publish-web`
- Not platform ops → `publish-admin`
...
Traefik routes tenant domain to the `publish-studio` Coolify app. API may be same-origin
...
| `NEXT_PUBLIC_OAUTH_CLIENT_ID` | Yes | `publish-tenant-frontend` |
...
NEXT_PUBLIC_OAUTH_CLIENT_ID=publish-tenant-frontend
# OAUTH_CLIENT_SECRET=publish-tenant-frontend-secret
```
...
- No registration link for editors — invite-only for `EDITOR`/`TENANT_ADMIN`
  (subscribers register on `publish-web`)
...
[`content-creation-implementation.md` § 5](content-creation-implementation.md#5-frontend-implementation-publish-studio).
...
3. **OAuth2** — `publish-tenant-frontend` client; token refresh before expiry
...
| Concern | Value |
|---------|-------|
| Project | `projects/publish-studio/` |
...
- Product overview: [`publish-studio.md`](publish-studio.md)
...
- Platform superadmin (separate app): [`publish-admin-implementation.md`](publish-admin-implementation.md)

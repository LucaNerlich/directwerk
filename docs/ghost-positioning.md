# Publish vs Ghost — Positioning

Companion to [`README.md`](../README.md) (full product design). This document answers a recurring
strategic question: **are we rebuilding Ghost, and if not, how do we position Publish against it?**
...
| [`product-naming.md`](product-naming.md) | Public product name — criteria and shortlist (Publish is codename only) |
| **This document** | Competitive positioning vs [Ghost](https://ghost.org/) and related products |

**Short answer:** No — we are not rebuilding Ghost. Publish is **whitelabel podcast membership
...
**Status (2026-07):** Publish is in **pre-implementation** (alpha POC spec only). Ghost is a mature,
...
| **Publish** | API-first, multi-tenant whitelabel infrastructure — tenants bring brand and frontend; we provide podcast content, entitlements, private RSS, and Patreon/Steady/Stripe plumbing |

**Publish in a sentence for customers:** *“Own your podcast membership stack on your domain — without renting Patreon, Steady, or a closed CMS.”* (from [`README.md` § Intro](../README.md#intro))
...
flowchart TB
    ...
    subgraph publish [Publish — infrastructure API]
...
| **Product shape** | Monolithic app (admin UI + member portal + themes) | **API + bundled apps** (`publish-studio`, `publish-web`); custom frontends for agencies |
...
Publish’s surface area, not the whole product.

### Where Publish is intentionally different (our moat)
...
| Ghost strength | Publish stance |
|----------------|----------------|
| Editor, themes, newsletter delivery | **Non-goal** for MVP — integrate via webhooks/email provider later; not a Ghost-style CMS |
| Polished member Portal out of the box | Ship **reference** `publish-web` per deal; API remains contract |
...
Publish’s README already rejects that shape:
...
| Product | Overlap with Publish | Key difference |
|---------|---------------------|----------------|
...
## When to recommend Ghost vs Publish
...
A tenant *could* use Ghost for marketing/blog and Publish for podcast membership API — two systems,
two bills, integration burden. **Not a default recommendation.** If blog is essential, Ghost wins;
if podcast membership infrastructure is essential, Publish wins. Pick a primary system of record.
...
| General-purpose block editor | `publish-studio` is ops UI, not a CMS editor |
...
[`publish-studio-implementation.md`](publish-studio-implementation.md)). If we add them, treat as **lightweight
...
| **P4 — Convenience** | `publish-studio` v0–v2 + `publish-web` as bundled default | Approaches Ghost’s “it just works” UX for creators |
...
2. **Protect the API contract** — every Ghost-like convenience in `publish-studio` must call the
...
**Elevator pitch (internal):** Publish is **whitelabel podcast membership infrastructure** — not a
...
| # | Question | Notes |
|---|----------|-------|
| 1 | Do we need a minimal **marketing site generator** (not full CMS) for tenants without a frontend? | `site-config` + static `publish-web` may suffice |
...
- Publisher dashboard (ops UI, not CMS): [`publish-studio-implementation.md`](publish-studio-implementation.md)
